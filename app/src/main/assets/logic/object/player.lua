...
function Player:Init()
	...
	self.crystal = 0;
	self.red_crystal = 0;
	self.online_state = ENUM.EOnlineState.offline;
    ...
    self.inviteList = {};
	self.IsYueKa = 0 -- 0为否 1为是
	self.IsYueKa1 = 0;
	self.get_friend_ap_times = 0
	self.fight_value = 0
	self.rollnamelist = {};
	self.IsRealNameAuth = -1; -- -1未请求 0已实名认证  1未实名认证
	self.IsRunGameTime = 0;  --当天的游戏运行时间
	self.is_get_bind_awards = 0;
	self.change_name_times = 0;  --改名次数
	self.peakshow = 0;
	self.churchVigor = 0; --教堂挂机精力
	self.gmSwitch = 0;
	self.heroTrialTicket = 0;
