...
PlayerEnterUITimesCurDay = 
{
    ...
    playerRunGameTimesIs3Hours = "playerRunGameTimesIs3Hours.data"
}
...
--设置是否满3小时   --true 满3小时  false 没满
function PlayerEnterUITimesCurDay.SetGameTimeDate(value)
    if value ~= nil then
        local file_handler = file.open(PlayerEnterUITimesCurDay.playerRunGameTimesIs3Hours,4);
        file_handler:write_string( "return "..tostring(value) );
        file_handler:close()        
    end
end

function PlayerEnterUITimesCurDay.GetGameTimeData()
    local result = false
    local file_handler = file.open_read(PlayerEnterUITimesCurDay.playerRunGameTimesIs3Hours);
    if file_handler then
...
function PlayerEnterUITimesCurDay.IsRestGameTimeData()

	

    local id = "co"..UserCenter.get_accountid()
    if not PlayerEnterUITimesCurDay.HasEnterUITo12(id) then
        PlayerEnterUITimesCurDay.SetGameTimeDate(false)
    end
end

function PlayerEnterUITimesCurDay.isShowLoginRenameWnd()
    if PlayerEnterUITimesCurDay.GetGameTimeData() then
