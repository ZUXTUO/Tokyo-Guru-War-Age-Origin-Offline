SevenSignMuBiaoBack = Class('SevenSignMuBiaoBack', UiBaseClass);
...
function SevenSignMuBiaoBack:Init(data)
...
function SevenSignMuBiaoBack:Restart()
    if UiBaseClass.Restart(self, data) then
	--todo 
	end
end

function SevenSignMuBiaoBack:InitData(data)
...
function SevenSignMuBiaoBack:DestroyUi()
...
function SevenSignMuBiaoBack:Show()
	if UiBaseClass.Show(self) then
	--todo 
	end
end

function SevenSignMuBiaoBack:Hide()
    if UiBaseClass.Hide(self) then
	    
	end
end

function SevenSignMuBiaoBack:RegistFunc()
...
--注册消息分发回调函数
function SevenSignMuBiaoBack:MsgRegist()
	UiBaseClass.MsgRegist(self);

	PublicFunc.msg_regist(msg_sign_in.gc_total_state_back, self.bindfunc["on_get_back_data"]);
    PublicFunc.msg_regist(msg_sign_in.gc_get_award_total_back, self.bindfunc["on_get_award_total"]);
end

--注销消息分发回调函数
function SevenSignMuBiaoBack:MsgUnRegist()
	UiBaseClass.MsgUnRegist(self);
    
    PublicFunc.msg_unregist(msg_sign_in.gc_total_state_back, self.bindfunc["on_get_back_data"]);
	PublicFunc.msg_unregist(msg_sign_in.gc_get_award_total_back, self.bindfunc["on_get_award_total"]);
end


function SevenSignMuBiaoBack:InitUI(asset_obj)
...
function SevenSignMuBiaoBack:ShowNavigationBar()
    return false;
end

function SevenSignMuBiaoBack:on_close( t )
	-- uiManager:RemoveUi(EUI.SevenSignMuBiaoBack);
	uiManager:PopUi(EUI.SevenSignMuBiaoBack);

end

function SevenSignMuBiaoBack:on_award_total_click( t )
	if self.current_index ~= 0 then
		msg_sign_in.cg_get_award_total_back(self.current_index, self.current_score);
	end
end

function SevenSignMuBiaoBack:set_deff_time( )
...
function SevenSignMuBiaoBack:on_get_back_data( is_get_total, c_score, timeLeft )
...
function SevenSignMuBiaoBack:on_get_award_total( t_index )
	local config_data = ConfigManager.Get(EConfigIndex.t_sign_in_total_award_back, t_index);
	local award = {};
