SevenSignUi = Class('SevenSignUi', UiBaseClass);
...
function SevenSignUi:Init(data)
...
function SevenSignUi:Restart()
	if self.current_type1 and self.current_type2 then
		-- msg_sign_in.cg_request_task_list(self.current_type1, self.current_type2);
	end
...
function SevenSignUi:InitData(data)
...
function SevenSignUi:DestroyUi()
...
function SevenSignUi:Hide()
    if UiBaseClass.Hide(self) then
	    
	end
end

function SevenSignUi:RegistFunc()
...
--注册消息分发回调函数
function SevenSignUi:MsgRegist()
	UiBaseClass.MsgRegist(self);

    PublicFunc.msg_regist(msg_sign_in.gc_task_list, self.bindfunc["on_set_list_by_data"]);
    PublicFunc.msg_regist(msg_sign_in.gc_get_award, self.bindfunc["on_award_back"]);
    PublicFunc.msg_regist(msg_sign_in.gc_set_point, self.bindfunc['SetUpdateUi'])

    PublicFunc.msg_regist(msg_sign_in.gc_total_state, self.bindfunc["on_get_back_data"]);
    PublicFunc.msg_regist(msg_sign_in.gc_set_point_states, self.bindfunc["gc_set_point_states"]);
end

--注销消息分发回调函数
function SevenSignUi:MsgUnRegist()
	UiBaseClass.MsgUnRegist(self);
    
    PublicFunc.msg_unregist(msg_sign_in.gc_task_list, self.bindfunc["on_set_list_by_data"]);
    PublicFunc.msg_unregist(msg_sign_in.gc_get_award, self.bindfunc["on_award_back"]);
    PublicFunc.msg_unregist(msg_sign_in.gc_set_point, self.bindfunc['SetUpdateUi'])

    PublicFunc.msg_unregist(msg_sign_in.gc_total_state, self.bindfunc["on_get_back_data"]);
    PublicFunc.msg_unregist(msg_sign_in.gc_set_point_states, self.bindfunc["gc_set_point_states"]);
end


function SevenSignUi:InitUI(asset_obj)
	...
	self.sp_title1_texture:set_texture("assetbundles/prefabs/ui/image/backgroud/kai_fu/kf_wenzidiban.assetbundle");
    ...
    --初始本地数据
    self.config_all_data = ConfigManager._GetConfigTable(EConfigIndex.t_sign_in_7);
    ...
    sp_point:set_active(g_dataCenter.signin:GetRedPointStateByType1(1));
    ...
    for i = 2, 5 do
    	if i <= self.type1_num then
			...
			sp_point:set_active(g_dataCenter.signin:GetRedPointStateByType1(i));
	...
	if not g_dataCenter.signin:GetVecDataByType1Type2(self.current_type1, self.current_type2) or g_dataCenter.signin:GetRedPointStateByType1AndType2(self.current_type1, self.current_type2) then
		msg_sign_in.cg_request_task_list(self.current_type1, self.current_type2);
	else
		local sTime, eTime = g_dataCenter.signin:GetTime();
		self:on_set_list_by_data(sTime, eTime, self.current_type1, self.current_type2, g_dataCenter.signin:GetVecDataByType1Type2(self.current_type1, self.current_type2) );
	end
	-- msg_sign_in.cg_get_award(90000001);
...
function SevenSignUi:UpdateCurrentType1Button( p_index )
	...
	for k,v in pairs(self.m_type1_item_ui) do
		...
		sp_point:set_active(g_dataCenter.signin:GetRedPointStateByType1(k));
...
function SevenSignUi:UpdateUiForBackData()
	...
	g_dataCenter.signin:SetRedPoint(is_state == 1);
...
function SevenSignUi:init_item_wrap_content( obj, b, real_id )
    ...
    local item_config_data = ConfigManager.Get(EConfigIndex.t_sign_in_7, item_back_data.task_id);
...
function SevenSignUi:UpdateUIToType2( )
	-- 左边页卡
	...
	for i = 1, 3 do
    	...
    	if i <= self.current_type2_num then
			...
			sp_point:set_active(g_dataCenter.signin:GetRedPointStateByType1AndType2(self.current_type1, i));
		else
...
function SevenSignUi:InitType1Config( )
	self.config_data_type1 = {};
    local type1 = 0;
...
function SevenSignUi:refeshCurrentType2ConfigByType1( type1 )
    -- 左边小类
...
function SevenSignUi:on_switch_right_type1( t )
...
function SevenSignUi:on_switch_top_type2( t )
...
function SevenSignUi:on_btn_click( t )
	...
	elseif state == 1 then
		msg_sign_in.cg_get_award(task_id);
	elseif state == 2 then
...
function SevenSignUi:on_btn_mu_biao_click( t )
	uiManager:PushUi(EUI.SevenSignMuBiao);
end

function SevenSignUi:on_close( t )
	uiManager:RemoveUi(EUI.SevenSignUi);
end

function SevenSignUi:set_deff_time( )
...
-------- 数据返回
function SevenSignUi:on_set_list_by_data( startTime, endTime, type1, type2, taskList )
...
function SevenSignUi:on_award_back( task_id )
...
function SevenSignUi:SetUpdateUi( )
	if g_dataCenter.signin:GetIsOpen() == false then
		uiManager:PopUi(EUI.SevenSignUi);
	end
end

function SevenSignUi:UpdateRedPoint( )
	local sp_point = nil;
	for k,v in pairs(self.m_type1_item_ui) do
		sp_point = ngui.find_sprite(self.yeka_right, "yeka" .. k .. "/sp_spint");
		if sp_point then
...
function SevenSignUi:on_get_back_data( is_get_total, c_score, timeLeft )
