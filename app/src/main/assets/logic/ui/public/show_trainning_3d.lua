Showtrainning3d = Class("Showtrainning3d");
Showtrainning3d.quotenum = 0;

-- local res = "assetbundles/prefabs/map/045_juesezhanshi/role3d.assetbundle";
local res = "assetbundles/prefabs/map/045_juesezhanshi/role3d_008.assetbundle";

function Showtrainning3d.GetResList()
	return {res}
end

function Showtrainning3d.SetAndShow(data)
    if Showtrainning3d.instance then
		Showtrainning3d.instance:UpdateData(data);
	else
		Showtrainning3d.instance = Showtrainning3d:new(data)
	end
end

function Showtrainning3d.SetLoadCallBack(callback)
	local self = Showtrainning3d.instance
	if self then
...
function Showtrainning3d.Destroy()
	Showtrainning3d.quotenum = Showtrainning3d.quotenum -1
	--app.log("quotenum============="..tostring(Show3d.quotenum))
	if Showtrainning3d.quotenum > 0 then
		return 
	end

    if Showtrainning3d.instance then
        Showtrainning3d.instance:Hide();
        Showtrainning3d.instance:DestroyUi();
        Showtrainning3d.instance = nil;
        Showtrainning3d.quotenum = 0;
    end
end

function Showtrainning3d.GetInstance()
	return Showtrainning3d.instance
end

function Showtrainning3d.Addquote( )

	Showtrainning3d.quotenum = Showtrainning3d.quotenum + 1
	-- body
end

function Showtrainning3d.SetVisible(bool)
	if Showtrainning3d.instance then
		Showtrainning3d.instance:SetVisibleRole3d(bool)
	end
end

function Showtrainning3d:Init(data)
...
function Showtrainning3d:InitData(data)
	self.pathRes = res;
	self.bindfunc = {};
...
function Showtrainning3d:RegistFunc()
	self.bindfunc['on_loaded'] = Utility.bind_callback(self, self.on_loaded);
end

--注销回调函数
function Showtrainning3d:UnRegistFunc()
...
function Showtrainning3d:on_loaded(pid, filepath, asset_obj, error_info)
	if filepath == self.pathRes then
		self:InitUI(asset_obj);
	end
end

function Showtrainning3d:InitUI(obj)
...
function Showtrainning3d:UpdateData(data)
...
function Showtrainning3d:SetVisibleRole3d(bVisible)
