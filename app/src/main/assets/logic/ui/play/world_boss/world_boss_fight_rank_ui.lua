WorldBossFightRankUI = Class('WorldBossFightRankUI', UiBaseClass);
--初始化
--data =
--{
--  parent = nil    ui挂接的父节点
...
function WorldBossFightRankUI:Init(data)
    self.pathRes = "assetbundles/prefabs/ui/new_fight/new_fight_ui_world_boss.assetbundle";
...
--初始化数据
function WorldBossFightRankUI:InitData(data)
...
--注册回调函数
function WorldBossFightRankUI:RegistFunc()
    UiBaseClass.RegistFunc(self);
    self.bindfunc['OnInOut'] = Utility.bind_callback(self, self.OnInOut);
    self.bindfunc['UpdateReviveUi'] = Utility.bind_callback(self, self.UpdateReviveUi);
end

--初始化UI
function WorldBossFightRankUI:InitUI(asset_obj)
    ...
    leftOther["labOwnValue"]:set_text("[00C0FFFF]您的伤害[-] "..tostring(0));
    ...
    leftOther["labSingleDamage"] = ngui.find_label(self.ui, "lab_harm");
    leftOther["labSingleDamage"]:set_text("[00C0FFFF]您的单次伤害[-] 0");
    leftOther["spSingleDamage"] = ngui.find_sprite(self.ui,"lab_harm/sp_s");
    ...
    if FightScene.GetPlayMethodType() == MsgEnum.eactivity_time.eActivityTime_WorldBoss then
        ...
        if g_dataCenter.worldBoss:GetState() == 1 then
            local time = g_dataCenter.worldBoss:GetLastTime();
            if time > 0 then
                self:SetReviveUI(time, g_dataCenter.worldBoss:GetKillerName())
            end
        end
    ...
    self.nodeCountdown = self.ui:get_child_by_name("animation/sp_bk");
    self.nodeCountdown:set_active(false);
    self.labPlayerName = ngui.find_label(self.ui,"sp_bk/lab_name");
    self.labTime = ngui.find_label(self.ui,"sp_bk/lab_time");

    self.isOut = false;
end
--析构函数
function WorldBossFightRankUI:DestroyUi()
...
--排行榜更新
function WorldBossFightRankUI:UpdateRank(rankInfo, demage, rankPlaces, singleDamage)
    self.data.rankInfo = rankInfo or self.data.rankInfo or {};
    self.data.demage = demage or self.data.demage or 0;
    self.data.singleDamage = singleDamage or self.data.singleDamage or 0;
    ...
    for i = 1, showRankNum do
        if worldBossData.rankInfo then
            ...
            if worldBossData.rankInfo[nSize - i + 1] then
                ...
                leftOther["labPlaces"..i.."Name"]:set_text(tostring(worldBossData.rankInfo[i].player_name));
    ...
    leftOther["labSingleDamage"]:set_text("[00C0FFFF]您的单次伤害[-] "..tostring(self.data.singleDamage))
    local lv = g_dataCenter.worldBoss:GetDamageLv(g_dataCenter.player.level, self.data.singleDamage);
    leftOther["spSingleDamage"]:set_sprite_name(EDamageLv[lv]);
...
function WorldBossFightRankUI:SetReviveUI(time, playerName)
    if time == 0 then
        ...
        self.timerId = timer.create(self.bindfunc["UpdateReviveUi"], 1000, time+1);
        self.reviveTime = time;
        self.playerName = playerName;
    end
    self:UpdateReviveUi();
end

function WorldBossFightRankUI:UpdateReviveUi()
    self.reviveTime = self.reviveTime - 1;
    if not self.nodeCountdown then return end;
    if self.timerId and self.reviveTime >= 0 then
        self.nodeCountdown:set_active(true);
        self.labPlayerName:set_text(self.playerName);
        self.labTime:set_text(tostring(self.reviveTime));
    else
        self.nodeCountdown:set_active(false);
        self.timerId = nil;
    end
...
function WorldBossFightRankUI:OnInOut()
    local leftOther = self.control;
