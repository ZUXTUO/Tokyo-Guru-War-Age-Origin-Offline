TotalConsumeUI = Class("TotalConsumeUI", UiBaseClass)
...
function TotalConsumeUI:DestroyUi()
...
function TotalConsumeUI:RegistFunc()
    ...
    self.bindfunc["gc_back_total_consume_state"] = Utility.bind_callback(self, self.gc_back_total_consume_state);
    self.bindfunc["gc_back_total_consume_award"] = Utility.bind_callback(self, self.gc_back_total_consume_award);

    self.bindfunc["set_deff_time"] = Utility.bind_callback(self, self.set_deff_time);
    
end

function TotalConsumeUI:MsgRegist()
	PublicFunc.msg_regist("msg_activity.gc_back_total_consume_state", self.bindfunc["gc_back_total_consume_state"]);
	PublicFunc.msg_regist("msg_activity.gc_back_total_consume_award", self.bindfunc["gc_back_total_consume_award"]);
end

function TotalConsumeUI:MsgUnRegist()
	PublicFunc.msg_unregist("msg_activity.gc_back_total_consume_state", self.bindfunc["gc_back_total_consume_state"]);
	PublicFunc.msg_unregist("msg_activity.gc_back_total_consume_award", self.bindfunc["gc_back_total_consume_award"]);
end

function TotalConsumeUI:InitUI(asset_obj)
	UiBaseClass.InitUI(self, asset_obj)
	-- self.ui:set_parent(Root.get_root_ui_2d());
    self.ui:set_parent(Root.get_root_ui_2d():get_child_by_name("activity_ui"));
	self.ui:set_name("total_consume_ui");
    ...
    self.sp_di = ngui.find_sprite(self.ui, "total_consume_ui/center_other/animation/sp_di");
    self.sp_di:set_active(false);
    self.sp_mark = ngui.find_sprite(self.ui, "total_consume_ui/center_other/animation/sp_mark");
    ...
    self.contentPath = "total_consume_ui/center_other/animation/content/";
    self.sp_art_font = ngui.find_sprite(self.ui, self.contentPath .. "sp_art_font");
    if self.sp_art_font then
        self.sp_art_font:set_sprite_name("hd_biaoti_xiaofei");
    end
    ...
    local activityTime = g_dataCenter.activityReward:GetActivityTimeForActivityID(ENUM.Activity.activityType_consume_total);
    ...
    self.grid = ngui.find_wrap_content(self.ui,"total_consume_ui/center_other/animation/panel/wrap_content");
    self.grid:set_on_initialize_item(self.bindfunc["init_item"]);
    self.grid:set_min_index(0);
    self.grid:set_max_index(-1);
    self.grid:reset();
    self.gridScrollView = ngui.find_scroll_view(self.ui, "total_consume_ui/center_other/animation/panel")

    msg_activity.cg_get_total_consume_state();
end

function TotalConsumeUI:set_deff_time( )
...
function TotalConsumeUI:init_item(obj, b, real_id)
...
function TotalConsumeUI:UpdateRewardUI()
...
function TotalConsumeUI:Restart(data)
    self:InitData(data)
...
function TotalConsumeUI:on_get_award( t )
	if t.string_value == "1" then
		msg_activity.cg_get_total_consume_award(t.float_value);
	end
end

function TotalConsumeUI:gc_back_total_consume_state( start_time, end_time, vecStates )
    ...
    g_dataCenter.activityReward:SetRedPointStateByActivityID(ENUM.Activity.activityType_consume_total, is_red_point_state);
...
function TotalConsumeUI:gc_back_total_consume_award( tId, award )
	-- local item_config_data = ConfigManager.Get(EConfigIndex.t_total_cunsume, tId);
	-- local award = {};
