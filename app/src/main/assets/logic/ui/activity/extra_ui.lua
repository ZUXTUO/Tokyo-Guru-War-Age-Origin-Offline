ExtraUI = Class("ExtraUI", UiBaseClass);

local _lab_txt = {
	[1] = "%d月%d日%02d:00~%d月%d日%02d:00";
}

function ExtraUI:Init(data)
	self.m_activity_id = data.id;
    self.pathRes = "assetbundles/prefabs/ui/award/ui_1130_award.assetbundle";
    UiBaseClass.Init(self, data);
end

function ExtraUI:InitData(data)
    UiBaseClass.InitData(self, data);    

    self.updateTimer = 0;
end

function ExtraUI:Restart(data)
	
	UiBaseClass.Restart(self, data);
end

function ExtraUI:DestroyUi()
...
function ExtraUI:InitUI(asset_obj)
...
function ExtraUI:set_deff_time( )
...
function ExtraUI:on_go( t )
	local enter_id = MsgEnum.eactivity_time.eActivityTime_LevelActivity;
	if self.m_activity_id == ENUM.Activity.activityType_extra_times_hight_sniper then
		enter_id = MsgEnum.eactivity_time.eActivityTime_baoWeiCanChang;
	elseif self.m_activity_id == ENUM.Activity.activityType_extra_times_baoweizhang then
		enter_id = MsgEnum.eactivity_time.eActivityTime_gaoSuJuJi;
	elseif self.m_activity_id == ENUM.Activity.activityType_extra_times_xiaochoujihua then
		enter_id = MsgEnum.eactivity_time.eActivityTime_ClownPlan;
	elseif self.m_activity_id == ENUM.Activity.activityType_hurdle_extra_produce then
		enter_id = MsgEnum.eactivity_time.eActivityTime_Adventure;
	end
