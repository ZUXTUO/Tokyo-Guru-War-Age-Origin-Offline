...
public class UILabel : UIWidget
{
	...
	[HideInInspector]
	[SerializeField]
	private bool bUseResizeHeightEx;
	...
	protected void SetActiveFont(Font fnt)
	{
		...
		int value;
		if (mActiveTTF != null && mFontUsage.TryGetValue(mActiveTTF, out value))
		{
	...
	public override void OnFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		if (effectStyle != 0)
		{
			...
			if (effectStyle == Effect.Outline || effectStyle == Effect.Outline8 || effectStyle == Effect.Outline16)
			{
				num = size2;
				size2 = verts.size;
				...
				if (effectStyle == Effect.Outline16)
				{
					num = size2;
					size2 = verts.size;
					ApplyShadow(verts, uvs, cols, num, size2, 0f - vector.x, vector.y);
					for (int i = 0; (float)i < vector.x * 2f; i++)
					{
						num = size2;
						size2 = verts.size;
						ApplyShadow(verts, uvs, cols, num, size2, vector.x - (float)i, vector.y);
					}
					num = size2;
					size2 = verts.size;
					ApplyShadow(verts, uvs, cols, num, size2, vector.x, vector.y);
					for (int j = 0; (float)j < vector.y * 2f; j++)
					{
						num = size2;
						size2 = verts.size;
						ApplyShadow(verts, uvs, cols, num, size2, vector.x, vector.y - (float)j);
					}
					num = size2;
					size2 = verts.size;
					ApplyShadow(verts, uvs, cols, num, size2, vector.x, 0f - vector.y);
					for (int k = 0; (float)k < vector.x * 2f; k++)
					{
						num = size2;
						size2 = verts.size;
						ApplyShadow(verts, uvs, cols, num, size2, vector.x - (float)k, 0f - vector.y);
					}
					num = size2;
					size2 = verts.size;
					ApplyShadow(verts, uvs, cols, num, size2, 0f - vector.x, 0f - vector.y);
					for (int l = 0; (float)l < vector.y * 2f; l++)
					{
						num = size2;
						size2 = verts.size;
						ApplyShadow(verts, uvs, cols, num, size2, 0f - vector.x, vector.y - (float)l);
					}
				}
			}
		}
	...
	[ContextMenu("Set Shadow Style")]
	public void SetDefaultValue()
	{
		fontSize = 26;
		keepCrispWhenShrunk = Crispness.Never;
		mApplyGradient = false;
		effectStyle = Effect.Outline;
		mEffectDistance.y = 1f;
		mEffectDistance.x = 1f;
		base.color = Color.white;
		effectColor = new Color(0.11372549f, 1f / 3f, 32f / 51f, 1f);
		base.depth = 450;
		mOverflow = Overflow.ClampContent;
		base.height = 26;
	}

	[ContextMenu("Set Shadow Style1")]
	public void SetDefaultValue1()
	{
		fontSize = 26;
		keepCrispWhenShrunk = Crispness.Never;
		mApplyGradient = false;
		...
		mEffectDistance.x = 1f;
		base.color = Color.white;
		effectColor = new Color(58f / 85f, 13f / 51f, 8f / 51f, 1f);
		base.depth = 450;
		mOverflow = Overflow.ClampContent;
		base.height = 26;
	}
